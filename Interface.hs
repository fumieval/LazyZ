...
stdout :: Expr e -> IO ()
stdout = putStr . map chr . takeWhile (<256) . decodeNat . split
-}
...
sequencial action expr = sequencial' (apply expr K) (apply expr (K :$ I))
    where
        sequencial' x f = (action M.! decodeNat (apply x K)) (toList $ apply x (K :$ I)) >>= sequencial action . apply f

listenOnFromExpr :: Expr e -> IO (Expr Socket)
listenOnFromExpr port = Extern <$> listenOn (PortNumber $ toEnum $ decodeNat port)

acceptFromExpr :: Expr Socket -> IO (Expr Handle)
acceptFromExpr (Extern sock) = encode <$> accept sock
    where
        encode (h, host, port) = fromList [Extern h, fromString host, encodeNat (fromEnum port)]
...
connectToFromExpr host port = Extern <$> connectTo (toString host) (PortNumber $ toEnum $ decodeNat port)
